...
data FT v a = Empty 
            ...
            | Deep {
                   ...
                   , suffix     :: Affix a }  deriving Eq -- (Eq,Show) 
...
data Node v a = Br2 v a a 
              | Br3 v a a a  deriving Eq -- (Eq,Show)

instance (Show a) => Show (Node v a) where
  show (Br3 _ a b c) = show a ++ show b ++ show c
  show (Br2 _ a b)   = show a ++ show b 

data Affix a  = One   a 
              | Two   a a 
              | Three a a a 
              | Four  a a a a  deriving (Eq,Show)  
...
instance Measured a v => IsList (Node v a) where 
   ...
   toList (Br2 _ x y)   = [x,y]   
   toList (Br3 _ x y z) = [x,y,z] 

   fromList [x,y]   = Br2 (measure x <> measure y) x y 
   fromList [x,y,z] = Br3 (measure x <> measure y <> measure z) x y z 
...
affix2FT :: Measured a v =>  Affix a -> FT v a 
...
nodes [x,y]    = [Br2 (measure x <> measure y)  x y] 
nodes [x,y,z]  = [Br3 (measure x <> measure y <> measure z) x y z] 
nodes (x:y:zs) = (Br2 (measure x <> measure y) x y) : nodes zs 
